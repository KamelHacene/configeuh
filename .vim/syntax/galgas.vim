...
"  GALGAS SPECIFIC
"
syn keyword	galgasKeywords	list style error message rule insert search in default send repeat end select drop or log map abstract class method override unused filewrapper template let
syn region	galgasType		start="@" end="[^a-zA-Z0-9.]"me=e-1 end="$"

syn region	galgasArgument	start="!" end="[^a-zA-Z0-9@.]"me=e-1 end="$"
    "Add 'contains=galgasType' to color the type
syn region	galgasArgument	start="?" end="[^a-zA-Z0-9@.]"me=e-1 end="$"
    "Add 'contains=galgasType' to color the type

syn region	galgasComments	start="#" end="$" contains=cTodo
syn region	galgasSymbol	matchgroup=galgasSymbolStart start="\$" end="\$" extend
syn region	galgasFunction	matchgroup=galgasSymbolStart start="\[" end="\]" contains=galgasArgument,galgasFunction,cString extend keepend
syn region	galgasRoutine	start="<" end=">" extend

" The colors are chosen based on jellybeans colorscheme.
" Use :hi command and change the colors as it suits you
hi def link galgasSymbolStart	Constant
hi def link galgasSymbol		PreProc
hi def link galgasType			Type
hi def link galgasArgument		Identifier
hi def link galgasComments		Comment
hi def link galgasKeywords		Label
hi def link galgasRoutine		Special
