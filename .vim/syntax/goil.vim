...
"  GOIL SPECIFIC
"
syn keyword	goilKeywords	if then else elsif end loop while foreach before after do mapof listof let template
syn keyword goilKeyvalues	true false emptymap emptylist
"syn region	goilComments	start="#" end="$" contains=cTodo
syn region	goilCommands	matchgroup=goilCommandStart start="%" end="%" contains=goilKeywords,goilKeyvalues,goilComments,cString

" The colors are chosen based on jellybeans colors.
" Use :hi command and change the colors as it suits you
hi def link goilKeyvalues		String
hi def link goilKeywords		Function
hi def link goilCommands		Float
hi def link goilCommandStart	Comment
hi def link goilComments		Comment
